...
export const formatBookingItineraire = (booking) => {
  const stops = Array.isArray(booking.trajetStops)
    ? booking.trajetStops.map((s) => (typeof s === "string" ? s.trim() : "")).filter((s) => s.length > 0)
    : [];

  if (stops.length === 0) {
    const rawTrajet = typeof booking.trajet === "string" ? booking.trajet.trim() : "";
    return rawTrajet ? rawTrajet : "Tunis";
  }

  if (stops.length === 1) {
    return stops[0] === "Tunis" ? "Tunis" : stops[0];
  }

  const routeStops = stops.filter((stop) => stop !== "Tunis");
  return routeStops.length > 0 ? routeStops.join(" → ") : "Tunis";
};
...
� *À envoyer avant l’événement :*
- Veuillez nous envoyer l’emplacement du marié et de la mariée via Google Maps.
- Veuillez aussi nous envoyer la 1ère destination de la journée.
